...
ThreadPool::ThreadPool(unsigned int numOfThreads, unsigned int numOfJobsPower, bool randomize) :
	m_hists(numOfThreads),
	m_numOfThreads(numOfThreads),
	m_jobsCounter(0),
	m_numOfJobs(1 << numOfJobsPower),
	...
	m_randomize(randomize)
{
	...
	uint64_t offset;
	if (numOfJobsPower >= N - 1)
	{
		offset = 1;
	}
	else
	{
		offset = (1 << (N - 1)) / (1 << numOfJobsPower);
	}

	uint64_t base = 0;
	while (base + offset < (1 << (N - 1)))
	{
		m_jobs.push(make_tuple(base, base + offset));
		base += offset;
	}
	m_jobs.push(make_tuple(base, (1 << (N - 1))));
...
* @param hist - a pointer to the histogram that this specific thread should add its results to.
* 
* This function has been optimized to save running time. For more details please check the documentation of SCSS::PairsToAdd.
*/
void doJob(uint64_t start, uint64_t end, unsigned int solutionArray[N + 1][N + 1], vector<uint64_t>* hist)
{
	for (auto s1 = start; s1 < end; ++s1)
	{
		...
		for (auto s2 = s1; s2 < (1 << N); ++s2)
		{
			...
			if (res != 0)
			{
				(*hist)[SCS::Calculate(SCS::Strand(s1), SCS::Strand(s2), solutionArray, prevS2) - N] += res;
				prevS2 = SCS::Strand(s2);
			}
		}
	}
}
...
void doRandomizedJob(unsigned int solutionArray[N + 1][N + 1], vector<uint64_t>* hist)
{
	for (auto i = 0; i < NUM_OF_RANDOMS_PER_JOB; ++i)
	{
		...
		(*hist)[SCS::Calculate(s1, s2, solutionArray, SCS::Strand(0)) - N]++;
	}
}
